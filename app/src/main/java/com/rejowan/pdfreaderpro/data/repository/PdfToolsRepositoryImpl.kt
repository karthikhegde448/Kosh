...
class PdfToolsRepositoryImpl(
    ...
    /**
     * Last-resort fallback: render each page via Android's PdfRenderer to a bitmap,
     * then write those bitmaps as an A4 PDF using iText. This bypasses all iText
     * content-stream parsing, so it works on every PDF that Android can display.
     * Used when iText copyPagesTo/direct operations fail with unknown version / content errors.
     */
    private fun renderToPdfFallback(inputPath: String, outputPath: String) {
        val fd = android.os.ParcelFileDescriptor.open(
            java.io.File(inputPath), android.os.ParcelFileDescriptor.MODE_READ_ONLY)
        ...
        for (i in 0 until renderer.pageCount) {
            ...
            // Read dimensions BEFORE closing the page
            ...
            // Add page to PDF
            val pageSize = PageSize(pageW.toFloat(), pageH.toFloat())
            val outPage = outDoc.addNewPage(pageSize)
            ...
            pdfCanvas.addImageFittedIntoRectangle(
                imgData,
                com.itextpdf.kernel.geom.Rectangle(0f, 0f, pageW.toFloat(), pageH.toFloat()),
                false
            )
        }
    ...
    /**
     * Strategy: try with original bytes first (lenient mode). If that throws
     * (unknown version, corrupt header, PDF 2.0 iText issues, etc.), patch the
     * header to %PDF-1.4 and retry. This covers all known "parser does not
     * support specification Unknown version" errors.
     */
    private fun openLenientPdf(path: String): PdfDocument {
        val bytes = File(path).readBytes()

        fun makeReader(data: ByteArray): PdfReader =
            PdfReader(ByteArrayInputStream(data), ReaderProperties())
                .setStrictnessLevel(PdfReader.StrictnessLevel.LENIENT)
                .also { it.setUnethicalReading(true) }

        fun patchTo14(raw: ByteArray): ByteArray {
            val header = String(raw.take(minOf(1024, raw.size)).toByteArray(), Charsets.ISO_8859_1)
            val idx = header.indexOf("%PDF-")
            return if (idx >= 0) {
                // Find end of version line (newline or space)
                var vEnd = idx + 5
                while (vEnd < idx + 12 && vEnd < header.length &&
                    header[vEnd] != '\n' && header[vEnd] != '\r' && header[vEnd] != ' ') vEnd++
                raw.slice(0 until idx).toByteArray() +
                    "%PDF-1.4\n".toByteArray(Charsets.ISO_8859_1) +
                    raw.drop(vEnd).toByteArray()
            } else {
                "%PDF-1.4\n".toByteArray(Charsets.ISO_8859_1) + raw
            }
        }

        // First attempt: use original bytes
        return try {
            PdfDocument(makeReader(bytes))
        } catch (e: Exception) {
            Timber.w("openLenientPdf first attempt failed (${e.message}), patching header: $path")
            // Second attempt: patch header to PDF-1.4
            try {
                PdfDocument(makeReader(patchTo14(bytes)))
            } catch (e2: Exception) {
                Timber.e(e2, "openLenientPdf failed even after header patch: $path")
                throw e2
            }
        }
    }
    ...
    /**
     * Same robustness for stamping-mode (reader+writer) operations.
     */
    private fun lenientReader(path: String): PdfReader {
        val bytes = File(path).readBytes()
        ...
        return try {
            ...
            Timber.w("lenientReader first attempt failed (${e.message}), patching header: $path")
            try {
                makeReader(patchTo14(bytes))
            } catch (e2: Exception) {
                Timber.e(e2, "lenientReader failed even after header patch: $path")
                throw e2
            }
        }
    }
    ...
    override suspend fun mergePdfs(
        ...
        try {
            ...
            inputPaths.forEachIndexed { index, path ->
                try {
                    val sourceDoc = openLenientPdf(path)
                    merger.merge(sourceDoc, 1, sourceDoc.numberOfPages)
                    sourceDoc.close()
                } catch (e: Exception) {
                    Timber.w("mergePdfs: iText failed for $path, using render fallback")
                    val tempPath = File(context.cacheDir, "merge_fallback_$index.pdf").absolutePath
                    renderToPdfFallback(path, tempPath)
                    val fallbackDoc = openLenientPdf(tempPath)
                    merger.merge(fallbackDoc, 1, fallbackDoc.numberOfPages)
                    fallbackDoc.close()
                }
    ...
    override suspend fun mergePdfsWithSelection(
        ...
        try {
            ...
            selections.forEachIndexed { index, selection ->
                val sourceDoc = openLenientPdf(selection.path)
    ...
    override suspend fun splitPdf(
        ...
        // If iText can't open this PDF, pre-convert it via render fallback
        val workPath = try {
            openLenientPdf(inputPath).also { it.close() }
            inputPath
        } catch (e: Exception) {
            Timber.w("splitPdf: using render fallback for $inputPath")
            val tmp = File(context.cacheDir, "split_fallback.pdf").absolutePath
            renderToPdfFallback(inputPath, tmp); tmp
        }
        try {
            val sourceDoc = openLenientPdf(workPath)
            val totalPages = sourceDoc.numberOfPages
    ...
    override suspend fun splitIntoPages(
        ...
        try {
            val sourceDoc = openLenientPdf(inputPath)
            val totalPages = sourceDoc.numberOfPages
            val createdFiles = mutableListOf<String>()
            val baseName = File(inputPath).nameWithoutExtension
    ...
    override suspend fun compressPdf(
        ...
        try {
            ...
            val totalPages = sourceDoc.numberOfPages
            sourceDoc.copyPagesTo(1, totalPages, destDoc)
            for (pageNum in 1..totalPages) onProgress(pageNum.toFloat() / totalPages)
            sourceDoc.close()
            destDoc.close()
            Result.success(File(outputPath).length())
        } catch (e: Exception) {
            Timber.w("compressPdf iText failed, using render fallback: ${e.message}")
            try {
                renderToPdfFallback(inputPath, outputPath)
                onProgress(1f)
                Result.success(File(outputPath).length())
            } catch (e2: Exception) {
                Timber.e(e2, "compressPdf render fallback also failed")
                Result.failure(e2)
            }
        }
    }
    ...
    override suspend fun getPageCount(inputPath: String): Result<Int> = withContext(Dispatchers.IO) {
        // Try iText first (handles most PDFs with lenient mode)
        try {
            val pdfDoc = openLenientPdf(inputPath)
            val count = pdfDoc.numberOfPages
            pdfDoc.close()
            return@withContext Result.success(count)
        } catch (e: Exception) {
            Timber.w("getPageCount iText failed, trying PdfRenderer: ${e.message}")
        }
        // Fallback: Android's PdfRenderer works on any valid PDF
        try {
            val fd = android.os.ParcelFileDescriptor.open(
            ...
            Timber.e(e2, "Failed to get page count: $inputPath")
            Result.failure(e2)
        }
    }
    ...
    override suspend fun lockPdf(
        ...
        try {
            ...
            val reader = lenientReader(inputPath)
    ...
    override suspend fun unlockPdf(
        ...
        try {
            ...
            // Open encrypted PDF with password — uses patched bytes + readerProperties

            // For password-protected PDFs, use readerProperties path with lenient + unethical
            val passwordReader = run {
                val file = File(inputPath)
                val bytes = file.readBytes()
                val headerStr = String(bytes.take(minOf(1024, bytes.size)).toByteArray(), Charsets.ISO_8859_1)
                val pdfMarkerIndex = headerStr.indexOf("%PDF-")
                val needsPatch = if (pdfMarkerIndex < 0) true else {
                    val v = headerStr.drop(pdfMarkerIndex + 5).take(3)
                    !(v.length >= 3 && v[0].isDigit() && v[1] == '.' && v[2].isDigit())
                }
                val stream = if (needsPatch) {
                    val patched = if (pdfMarkerIndex >= 0)
                        bytes.slice(0 until pdfMarkerIndex).toByteArray() +
                            "%PDF-1.4".toByteArray(Charsets.ISO_8859_1) +
                            bytes.drop(pdfMarkerIndex + 5).drop(3).toByteArray()
                ...
                PdfReader(stream, readerProperties)
                    .setStrictnessLevel(PdfReader.StrictnessLevel.LENIENT)
                    .also { r -> r.setUnethicalReading(true) }
            }
            ...
            val writer = PdfWriter(outputPath)
            val pdfDoc = PdfDocument(passwordReader, writer)
