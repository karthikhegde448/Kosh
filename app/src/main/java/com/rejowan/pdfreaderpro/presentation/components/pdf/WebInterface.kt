...
internal class WebInterface(private val pdfViewer: PdfViewer) {
    ...
    fun onFindMatchStart() = post {
        ...
        findSessionActive = true
        pdfViewer.listeners.forEach { it.onFindMatchStart() }
    }

    // Track whether a search session is active (started but not yet cleared)
    private var findSessionActive = false

    /** Called by FindController.stopFind() to clear the session state. */
    fun resetFindSession() {
        findMatchStarted = false
        findSessionActive = false
    }

    @JavascriptInterface
    fun onFindMatchChange(current: Int, total: Int) = post {
        // Always forward if a session is active - this covers findNext/findPrevious too
        if (findSessionActive) pdfViewer.listeners.forEach { it.onFindMatchChange(current, total) }
    }

    @JavascriptInterface
    fun onFindMatchComplete(found: Boolean) = post {
        if (findSessionActive) pdfViewer.listeners.forEach { it.onFindMatchComplete(found) }
        // Keep findSessionActive=true - session ends only when stopFind() is called
        // This allows findNext/findPrevious to keep receiving onFindMatchChange updates
    }
