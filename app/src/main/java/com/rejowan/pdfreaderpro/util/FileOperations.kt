...
object FileOperations {
    ...
    fun deleteFile(filePath: String, context: Context? = null): Boolean {
        return try {
            val file = File(filePath)
            // Try File.delete() first (works for app-private files)
            if (file.exists() && file.delete()) {
                Timber.d("Deleted via File.delete(): $filePath")
                return true
            }
            // On Android 10+ shared storage files need MediaStore deletion
            if (context != null && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                ...
                if (rows > 0) {
                    Timber.d("Deleted via MediaStore: $filePath")
                    return true
                }
            }
            // Last resort: try again with File (maybe it appeared after MediaStore op)
            if (file.exists()) {
                val result = file.delete()
                Timber.d("Final File.delete() result=$result for: $filePath")
                result
            } else {
                // File is already gone (MediaStore removed it)
                true
            }
        } catch (e: Exception) {
